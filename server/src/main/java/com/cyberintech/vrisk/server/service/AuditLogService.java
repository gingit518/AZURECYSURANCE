...
public class AuditLogService {
	...
	public FilteredResponse<AuditLogFilter, AuditLogViewDTO> getListFiltered(FilteredRequest<AuditLogFilter> filteredRequest) {
		...
		// Convert to DTOs
		List<AuditLogViewDTO> auditLogViewDTOS = AuditLogViewDTO.fromEntitiesList(pagedResult.getItems(), AuditLogViewDTO.class);
		...
		filteredResponse.setItems(auditLogViewDTOS);
	 ...
	 * @return Item Details
	 */
	public List<AuditLogViewExtendedDTO> searchItemAuditLogs(AuditLogFilter filter) {

		if (filter.getItemId() == null) {
			throw new BadRequestException("Entity ID is required for Audit search", ApplicationExceptionCodes.AUDIT_LOGS_MISSING_ITEM_ID_IN_SEARCH);
		}

		// Set Organization for Search
		filter.setOrganizationId(organizationService.getCurrentOrganizationId());

		PagedResult<AuditLog> pagedResult = auditLogModelDAO.getItemsPageable(filter, PageRequest.of(0, 1000000), BaseSort.of("id", BaseSort.SortOrder.ASC));

		// Convert to DTOs
		List<AuditLogViewExtendedDTO> result = AuditLogViewDTO.fromEntitiesList(pagedResult.getItems(), AuditLogViewExtendedDTO.class);
