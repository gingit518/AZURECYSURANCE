package com.cyberintech.vrisk.idp.service.spring;
...
public class CustomJdbcUserDetailsManager extends JdbcUserDetailsManager {
	...
	protected List<UserDetails> loadUsersByUsername(final String username) {
		List<UserDetails> result = userRepository.findAllByEmailIgnoreCase(username).stream().map(UserDetailsImpl::new)
	...
	protected UserDetails createUserDetails(String username, UserDetails userFromUserQuery,
		...
		UserDetailsImpl userDetails = new UserDetailsImpl(userFromUserQuery, combinedAuthorities,
			((UserDetailsImpl) userFromUserQuery).getUserId());
