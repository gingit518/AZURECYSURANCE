...
@RestController
@RequestMapping(
	value = TaskController.CONTROLLER_URI,
...
public class TaskController {
	...
	static final String CONTROLLER_URI = "/api/tasks";
	 ...
	 * @return Tasks List
	 */
	@RequestMapping(method = RequestMethod.POST, value = "/self", name = "Tasks List for current Filters and Risk Model")
	@Parameters({
		@Parameter(name = "authorization", description = "oAuth Access token for API calls", example = "Bearer DF0310", required = true, in = ParameterIn.HEADER)
	})
	@PreAuthorize("@apiSecurity.hasPermission(T(com.cyberintech.vrisk.api.config.APIAction).TASK_READ)")
	public FilteredResponse<TasksFilter, TaskBudgetViewDTO> getSelfListFiltered(
		@Parameter(description = "Item Filtering", required = true) @RequestBody FilteredRequest<TasksFilter> filteredRequest
	) {

		TasksFilter tasksFilter = filteredRequest.getFilter();
		if (tasksFilter == null) tasksFilter = new TasksFilter();
		tasksFilter.setTaskManagerOrAssigneeId(userService.getCurrentUser().getUserId());

		filteredRequest.setFilter(tasksFilter);
	...
	@PreAuthorize("@apiSecurity.hasPermission(T(com.cyberintech.vrisk.api.config.APIAction).TASK_READ)")
	public FilteredResponse<NameFilter, TaskBudgetViewDTO> getListFiltered(
		@Parameter(description = "Item Filtering", required = true) @RequestBody FilteredRequest<NameFilter> filteredRequest
	) {

		FilteredResponse<NameFilter, TaskBudgetViewDTO> result = taskService.getListFiltered(filteredRequest);
