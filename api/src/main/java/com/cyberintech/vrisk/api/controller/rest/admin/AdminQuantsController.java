...
@RestController
@RequestMapping(
	value = AdminQuantsController.CONTROLLER_URI,
	produces = MediaType.APPLICATION_JSON,
	name = "Admin Quants Management Controller"
)
@Tag(name = "Admin Quants Management")
public class AdminQuantsController {

	static final String CONTROLLER_URI = "/api/admin/quants";
	 ...
	 * @return Quants List
	 */
	@RequestMapping(method = RequestMethod.GET, value = "", name = "Quants List")
	@Parameters({
	 ...
	 * @return Quants List
	 */
	@RequestMapping(method = RequestMethod.POST, value = "/filter", name = "Quants List for current Filters")
	@Parameters({
		@Parameter(name = "authorization", description = "oAuth Access token for API calls", example = "Bearer DF0310", required = true, in = ParameterIn.HEADER)
	})
	@PreAuthorize("@apiSecurity.hasPermission(T(com.cyberintech.vrisk.api.config.APIAction).QUANTS_READ)")
	public FilteredResponse<NameFilter, QuantsViewDTO> getListFiltered(
		...
		FilteredResponse<NameFilter, QuantsViewDTO> result = quantsService.getListFiltered(null, filteredRequest);
	...
	@PreAuthorize("@apiSecurity.hasPermission(T(com.cyberintech.vrisk.api.config.APIAction).QUANTS_CREATE)")
	...
	@PreAuthorize("@apiSecurity.hasPermission(T(com.cyberintech.vrisk.api.config.APIAction).QUANTS_UPDATE)")
	 ...
	 */
	@RequestMapping(method = RequestMethod.DELETE, value = "", name = "Delete existing Quant Model", consumes = {MediaType.APPLICATION_JSON})
	@Parameters({
		@Parameter(name = "authorization", description = "oAuth Access token for API calls", example = "Bearer DF0310", required = true, in = ParameterIn.HEADER)
	})
	@PreAuthorize("@apiSecurity.hasPermission(T(com.cyberintech.vrisk.api.config.APIAction).QUANTS_DELETE)")
	public Long delete(
		@Parameter(description = "Simple Quant Details", required = true) @RequestBody ItemViewDTO itemDTO
	) {

		Long result = quantsService.delete(itemDTO.getId());
